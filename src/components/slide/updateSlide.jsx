...
export const UpdateSlide = ({deleteSlide, updateSlideDB, i, dataTabel, templateID, getSlide, isSite, check}) => {
    //const [checkBoxItem, setCheckBoxItem] = React.useState(dataTabel.checkboxes ? dataTabel.checkboxes : []);
    ...
    const { register, control, handleSubmit, reset, watch, formState: { errors } } = useForm({
        ...
        defaultValues: {
            name: dataTabel.name,
            description: dataTabel.name,
            checkboxes : dataTabel.checkboxes
        }
    });
    ...
    const [images, setImages] = React.useState(dataTabel.photos)
    const onDeleteSlide = async (idDeleteSlide) => {
        try {
            const date = {
                slide_id: idDeleteSlide
            }
            const obj = await SlidesApi.deleteSlidesApi(templateID, date)
            if (obj.statusCode === 200) {
                deleteSlide(idDeleteSlide)
            }
        } catch(error) {
            console.log(onDeleteSlide)
        }
    }
    ...
    React.useEffect(() => {
        reset({
            name: dataTabel.name,
            description: dataTabel.description,
            checkboxes: dataTabel.checkboxes,
            photos: dataTabel.photos,  
        })
    }, [dataTabel, check])
    
    const data = {
        slide_id: dataTabel.id,
        name: watch('name'),
        description: watch('description'),
        checkboxes: watch('checkboxes'),
        photos: images
    }
    ...
    return (
        <article className = "slide">
            <div className = "slide-header">
                ...
                <div className = "deleteBtn" onClick={() => onDeleteSlide(dataTabel.id)}>
                ...
                <div className = "slide-item">
                    ...
                    <div className="slide-item__list">
                        ...
                        <textarea
                            name = "description"
                            placeholder = "Текст.."
                            defaultValue = {dataTabel.description}
                ...
                <div className = "slide-item">
                    <p className ="slide-item__title"> Фото шаблона </p>
                        ...
                        <div className="slide-item__uploaded">
                            {images.length ? <p className ="slide-item__title"> Загруженные </p> : <></>}
